...
В отчете должен быть представлен текст SQL-запроса для создания параметризованной хранимой процедуры. 
Процедура должна воспринимать параметр – код объекта из какой-либо родительской таблицы базы данных и возвращать 
соответствующие записи из какой-либо дочерней таблицы (например, получать код студента и возвращать его оценки).
Формирование выходных данных в процедуре должно выполняться в четыре этапа:
1) Временная таблица должна быть создана с помощью CREATE TABLE. Временная таблица должна содержать 
все поля дочерней таблицы и дополнительно хотя бы одно поле из родительской таблицы (например, все поля таблицы Оценка + фамилию студента);
...
Drop Proc ОценкиУченика
Go

--Параметр - Код ученика
--Результат - Оценки ученика

Create Proc ОценкиУченика @КодУченика int As
  Create Table #result
    (Код int Primary Key
    , [Код ученика] int
    , ФИО varchar(80)
  ...
  Insert Into #result (Код, [Код ученика], [Код урока], Балл) 
    Select * From Оценки
      Where [Код ученика] = @КодУченика
  Update #result
    Set ФИО = (
      Select ФИО
        From Ученик
        Where Код = @КодУченика
    ) 
  Select * From #result
Go

Exec ОценкиУченика 3
...
В отчете должен быть представлен текст SQL-запроса для создания параметризованной хранимой процедуры. 
Процедура должна воспринимать параметр – код объекта из какой-либо родительской таблицы базы данных и 
возвращать соответствующие записи из какой-либо дочерней таблицы (например, получать код студента и возвращать его оценки).
Формирование выходных данных в процедуре должно выполняться в пять этапов:
1)      создание временной таблицы с помощью CREATE TABLE. Временная таблица должна содержать все поля дочерней таблицы и 
дополнительно хотя бы одно поле из родительской таблицы (например, все поля таблицы Оценка + фамилию студента);
...
4)      цикл ввода записей с помощью курсора. В теле цикла для каждой введенной записи выполняется запрос INSERT INTO … VALUES 
для занесения введенных данных во временную таблицу;
5)      возврат содержимого временной таблицы как результата работы процедуры.
Процедура на основе таблиц с данными по сотрудникам и должностям к рассмотрению не принимается.
В отчете также должен быть представлен скриншот по вызову разработанной процедуры в среде Management Studio.*/

Drop Proc ОценУченика
GO
 
--Параметр - Код ученика
--Результат - Оценки ученика

Create Proc ОценУченика @код int AS
  Create Table #res
  (Код int Primary Key
  ,[Код ученика] int
  ,[ФИО ученика] varchar(80)
  ,[Код урока] int
  ,Балл int
  )
Declare @фио varchar(80) = (
  Select ФИО 
    From Ученик
    Where Код = @код
  )
Declare Курсор Cursor for
  Select Код, [Код урока], Балл
    From Оценки
    Where @код = [Код ученика]

Open Курсор
Declare @код1 int, @код_урока int, @балл int
Fetch Курсор Into @код1, @код_урока, @балл
While @@FETCH_STATUS = 0
Begin
  Insert Into #res Values(@код1, @код, @фио, @код_урока, @балл)
  Fetch Курсор Into @код1, @код_урока, @балл
End
Close Курсор
Deallocate Курсор
...
«Разработка функции, возвращающей значение (скалярной функции)»
В отчете должен быть представлен текст SQL-запроса для создания функции, возвращающей значение. 
Функция должна воспринимать параметр – код объекта из какой-либо родительской таблицы базы данных и возвращать значение, рассчитанное по 
соответствующим записям из какой-либо дочерней таблицы (например, получать код студента и возвращать его средний балл).
Функция на основе таблиц с данными по сотрудникам и должностям к рассмотрению не принимается.
Также в отчете должен быть представлен скриншот по выполнению запроса в среде Management Studio для получения записей из 
родительской таблицы и соответствующего значения, рассчитанного функцией (кода, фамилии, группы студента и его среднего балла).*/

Drop function СредРезультат
Go

--Параметр - Код ученика
--Результат - Средний результат по олимпиадам ученика

Create function СредРезультат (@код int)
Returns int
  Begin
    Return (Select avg(Результат) From Олимпиада Where @код = [Код ученика])
  End
Go

Select *, dbo.СредРезультат(Код) As ResAvg
  From Ученик


/*ЛАБОРАТОРНАЯ РАБОТА № 19
«Разработка функции, возвращающей таблицу одиночным запросом»
В отчете должен быть представлен текст SQL-запроса для создания функции, возвращающей таблицу с помощью одиночного запроса (RETURNS TABLE). 
Функция должна воспринимать параметр – код объекта из какой-либо родительской таблицы базы данных и возвращать соответствующие записи из 
какой-либо дочерней таблицы (например, получать код студента и возвращать его оценки).
Функция на основе таблиц с данными по сотрудникам и должностям к рассмотрению не принимается.
Также в отчете должен быть представлен скриншот по выполнению запроса с вызовом разработанной функции в среде Management Studio.*/

Drop function ResOfPupil
Go

--Параметр - Код ученика
--Результат - Результаты ученика

Create function ResOfPupil (@код int)
Returns table
AS
Return
  Select Код, Название, [Код учителя], [Код предмета], Дата, Результат
    From Олимпиада
    Where [Код ученика] = @код
Go
...
«Разработка функции, возвращающей таблицу, полученную последовательностью операторов/запросов»
В отчете должен быть представлен текст SQL-запроса для создания функции, возвращающей таблицу, 
полученную путем выполнения последовательности операторов/запросов. 
Функция должна воспринимать параметр – код объекта из какой-либо родительской таблицы базы данных и возвращать таблицу, 
содержащую все поля дочерней таблицы, и дополнительно хотя бы одно поле из родительской таблицы 
(например, все поля таблицы Оценка + фамилию студента).
...
Drop function Результаты_Ученик
Go
-- Параметр - Код ученика.
-- Результат - Результаты олимпиады для указанного ученика.
Create function Результаты_Ученик (@код int)
Returns @res Table
  (Код int Primary Key
  ,Название varchar(80)
  ,[Код учителя] int
...
Select * From Результаты_Ученик(1)


/*ЛАБОРАТОРНАЯ РАБОТА № 21
«Разработка функции, возвращающей характеристику совокупности числовых значений»
В отчете должен быть представлен текст SQL-запроса для создания скалярной функции, возвращающей числовую характеристику, определенную  по совокупности числовых значений числового поля некоторой таблицы базы данных. Вариант условия лабораторной работы тот же, что и в работе №14.
Функция должна состоять из 3-х операторов:
1)      описания переменной для размещения результата;
2)      единственного SELECT-запроса к базе данных для получения требуемой характеристики и 
занесения значения в переменную (в запросе возможно использование подзапросов, в том числе в списке полей (возвращающих значение) и в предложении FROM (возвращающих набор данных));
3)      оператора RETURN.
Также в отчете должны быть представлены скриншоты по выполнению следующих действий:
1)      выполнения запроса для получения исходных данных из таблицы базы данных: поля первичного ключа и 
числового поля с элементами последовательности. Данные должны быть упорядочены по значению первичного ключа;
2)      выполнения запроса в среде Management Studio для вызова функции и отображения полученного значения.
Задание: 
4.Найти сумму элементов последовательности без двух последовательных элементов с минимальной суммой 
среди всех пар последовательных элементов*/
...
Begin
  ...
  Set @Сумма =
	...
	(Select min(Summa) 
      From (Select P1.Балл + P2.Балл AS Summa
	    From Оценки AS P1, Оценки AS P2
        Where P2.Код = (
		  Select min(Код)
		    From Оценки
		    Where Код > P1.Код)
...
Select Код, Балл From Оценки Order by Код
Go

Select dbo.Сумма() as Res


/*ЛАБОРАТОРНАЯ РАБОТА № 22
«Разработка триггеров для таблиц базы данных – поддержание ссылочной целостности»
Реализовать триггеры для поддержания ссылочной целостности базы данных. В отчете должны быть представлены SQL-запросы для 
удаления описания некоторых связей между таблицами и создания триггеров для замены этих описаний. 
Должны быть разработаны триггеры следующих видов:
...
В отчете также должны быть представлены скриншоты, демонстрирующие действие триггеров при работе с базой данных на 
сервере из MS Access:
1)      для каскадного удаления записей: содержимое дочерней таблицы до удаления записи родительской таблицы, 
удаление записи родительской таблицы,  содержимое дочерней таблицы после удаления записи родительской таблицы;
2)      для запрета на удаление: сообщение об ошибке при попытке удаления записи;
3)      для контроля наличия записи: соответствующие сообщения об ошибке (может выполняться в режиме отображения дочерней таблицы, 
когда код записи родительской таблицы вводится «руками»).*/
...
-- 1. Для каскадного удаления записей дочерней таблицы при удалении записи родительской таблицы.
Create trigger trgУрок_delete on Урок
  After delete
As
  Delete from Оценки 
    Where [Код урока] in (Select Код From deleted)
Go

-- 2. Запрета на удаление записи родительской таблицы при наличии связанных записей в дочерней таблице.
Create trigger trgКласс_delete on Класс
  After delete
As
  If exists(Select * from Ученик Where [Код класса] in (Select Код from deleted))
  Begin
    Raiserror('Нельзя удалить, так как имеются ученики.', 16, 2)
    Rollback tran
  End
Go

-- 3. Контроля наличия записи родительской таблицы при занесении записи дочерней таблицы.
Create trigger trgЗанесение_Оценок on Оценки
  After insert
As
  If exists(Select * from inserted where [Код ученика] not in (Select Код From Ученик))
  Begin
    Raiserror('Неверный код ученика.', 16, 2)
    Rollback tran
  End
Go
-- 4. Контроля наличия записи родительской таблицы при изменении кода этой записи в дочерней таблице.
Create trigger trgИзменение_Оценок on Оценки
  After update
As
  If update([Код ученика])
    If exists(Select * from inserted where [Код ученика] not in (Select Код from Ученик))
  Begin
    Raiserror('Неверный код ученика.', 16, 2)
    Rollback tran
...
Реализовать триггеры для накопления суммарной информации в таблице базы данных. 
В отчете должны быть представлены следующие SQL-запросы:
...
3)      создания триггеров для корректировки поля (пересчет значения поля СреднийБалл при корректировке таблицы Оценка).
В отчете также должны быть представлены скриншоты Management Studio, демонстрирующие действие триггеров:
1)      содержание таблицы после первичного заполнения;
2)      запросы на добавление, удаление, изменение записей таблицы, по которой выполняется расчет данных (таблицы Оценка) и 
новое содержимое поля суммарной информации. Запросы на удаление и изменение должны вызывать перерасчет поля 
более чем в одной записи.*/
...
Update Ученик
  Set kol =
    (Select count(*) From Оценки Where Ученик.Код = [Код ученика])
Go

Create trigger trgОценкиkol on Оценки
  After insert, update, delete
As
  Update Ученик
    Set kol -= k
	  From (Select [Код ученика], count(*) As k From deleted Group by [Код ученика]) As tabl
      Where Ученик.Код = [Код ученика]
  Update Ученик
    Set kol += k
	  From (Select [Код ученика], count(*) As k From inserted Group by [Код ученика]) As tabl
      Where Ученик.Код = [Код ученика]
Go
...
Реализовать триггеры для обновления данных через представление.
В отчете должны быть представлены следующие SQL-запросы:
1)      для создания представления, возвращающего все поля дочерней таблицы и поле родительской таблицы  
(все поля таблицы Оценка и фамилию студента);
2)      создания INSTEAD-триггеров для обновления дочерней таблицы, если обновление данных выполняется 
		для представления (при добавлении, удалении, изменении записей через представление изменяется таблица Оценка).
В отчете также должны быть представлены скриншоты Management Studio, демонстрирующие действие триггера:
1)      операторы добавления, удаления, изменения записей через представление;
2)      содержимое дочерней таблицы после выполнения запросов.*/

Drop View Оценки_Ученик
Go

Create View Оценки_Ученик
As
  Select Оценки.Код, [Код ученика], [Код урока], Балл, ФИО
    From Оценки, Ученик
    Where [Код ученика] = Ученик.Код
Go

Create Trigger trgОценки_УченикI on Оценки_Ученик Instead of Insert
As
  Insert Into Оценки
    Select [Код ученика], [Код урока], Балл
      From inserted
      Where ФИО is null
  Insert Into Оценки
    Select Ученик.Код, [Код урока], Балл
      From inserted, Ученик
      Where inserted.ФИО = Ученик.ФИО and [Код ученика] is null
  Insert Into Оценки
    Select Ученик.Код, [Код урока], Балл
      From inserted, Ученик
      Where [Код ученика] = Ученик.Код and inserted.ФИО = Ученик.ФИО and [Код ученика] is not null and inserted.ФИО is not null
Go

Create Trigger trgОценки_УченикU on Оценки_Ученик Instead of Update
As
  If not Update (ФИО)
    Update Оценки
      Set [Код ученика] = inserted.[Код ученика]
        , [Код урока] = inserted.[Код урока]
        , Балл = inserted.Балл
      From inserted
      Where inserted.Код = [Оценки].Код
  Else
    If not Update ([Код ученика])
      Update Оценки
        Set [Код ученика] = Ученик.Код
		  , [Код урока] = inserted.[Код урока]
          , Балл = inserted.Балл
        From inserted, Ученик
        Where inserted.Код = Оценки.Код and inserted.ФИО = Ученик.ФИО
    Else
      Update Оценки
        Set [Код ученика] = Ученик.Код
          , [Код урока] = inserted.[Код урока]
          , Балл = inserted.Балл
        From inserted, Ученик
        Where inserted.Код = Оценки.Код and inserted.ФИО = Ученик.ФИО and inserted.[Код ученика] = Ученик.Код
Go

Create Trigger trgОценки_УченикD on Оценки_Ученик Instead of Delete
As
  Delete From Оценки Where Код in (Select Код From deleted)
Go

Select * From Оценки
Go

Insert into Оценки_Ученик ([Код ученика], [Код урока], Балл, ФИО)
  Values(null, 2, 4, 'Ревунова Вероника Андреевна')
  ,(2, 4, 1, null)
  ,(3, 6, 5, 'Полховский Иван Павлович')
Go

Select * From Оценки
Go

Update Оценки_Ученик
  Set [Код ученика] = 3
  Where [Код ученика] = 2
Go
Update Оценки_Ученик
  Set ФИО = 'Ревунова Вероника Андреевна'
  Where ФИО = 'Губанова Александра Денисовна'
Go
Update Оценки_Ученик
  Set [Код ученика] = 2, ФИО = 'Ревунова Вероника Андреевна'
  Where [Код ученика] = 3
Go
